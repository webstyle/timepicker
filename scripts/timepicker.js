(function ($) {
   ...
   * @returns {{start: *, end: *}}
   */
  $.fn.timesPickerSave = function (callback) {
    ...
    if (formValue.length > 1) {
      ...
      switch (timePickerFormat) {
        case "24h":
          maxLength = 25;
          first.result = (first.result == 24) ? '00:00' : first.result;
          second.result = (second.result == 24) ? '00:00' : second.result;
          break;
        case "12h":
          maxLength = 13;
          break;
      ...
      if(first.hour < maxLength && second.hour < maxLength) {
  ...
  $.fn.timesPicker = function (options) {
    ...
    $(element).on('input', function (value) {
      ...
      var result;
      result = value.currentTarget.value.split(' - ');
        ...
        if (end) {
          ...
          $(element).bind("keydown", function (event) {
            ...
            select: function (event, ui) {
              ...
              var formValue, value, selectedObj;
